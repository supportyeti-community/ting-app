...
import {verifyServiceTickets} from '../tests/ting2/service-tickets.mjs';
import {verifyRoutingPreparation} from '../tests/ting2/routing-prepare.mjs';
const db=new PGlite();
...
await verifyServiceTickets(adapter,label=>console.log('PASS: '+label));
await verifyRoutingPreparation(adapter,label=>console.log('PASS: '+label));
await db.close();
